...
def s(m="Working"):
 ...
 def z():
  for c in itertools.cycle("⠋⠙⠹⠸⠼⠴⠦⠧⠇⠏"):
   if P.is_set(): break
   W(f"\r{m} {c} ",end="",flush=n);A(0.1)
 c=threading.Thread(target=z)
 c.start()
 try: yield
 finally: P.set();c.join();W(f"\r",end="",flush=n)#{m} Done\n
def X(u):
 W("\nExtracting image URLs from page...")
 with s("Fetching page"):
  try: R=q(u,headers=v);R.raise_for_status()
  except g as e: W(f"\n\033[31mError fetching page: {e}.\033[0m");return[]
  l=BeautifulSoup(R.text,"html.parser")
 S=[]
 for V in l.select("#reader img"):
  t=V.get("data-src")or V.get("src")
  if t: S.append(urljoin(L,t))
 W("Total images found:\033[33m",vL(S),"\033[0m")
 return S
def f(i,t,a=""):
 Q=i/t;Y=40;H=vP(Y*Q);y=f" {Q:^6.1%} ";w=Y//2
 C="█"*H+"░"*(w-H)+y+"░"*w if(Q<0.5) else "█"*w+y+"█"*(H-w)+"░"*(Y-H)
 W(f"\033[32m\r|{C}| \033[36m{i:>3}/{t} \033[0m{a} ",end="")
def G(S,D):
 W("\n📥 Downloading images...")
 ...
 for i,u in vc(S):
  try: r=q(u,headers=v);r.raise_for_status()
  except g as e: W(f"\nError downloading image {i+1}: \033[31m{e}.\033[0m");return[]
  o=D/f"{i:03}.jpg"
  with vR(o,"wb")as fv: fv.write(r.content)
  j.append(o)
...
def K(j,U):
 ...
 k=[];i=1;m=0
 for fv in j:
  V=Image.open(fv).convert("RGB");k.append(V)
  m+=fv.stat().st_size;f(i,vL(j),f" (\033[33m{m/gm:.2f} MB\033[0m)");i+=1
 W("\n")
 with s("Packaging PDF"):
  if k:k[0].save(U,save_all=n,append_images=k[1:])
 W("📁 PDF created:\033[33m",U,"\033[0m",f" (\033[36m{U.stat().st_size/gm:.2f} MB\033[0m)")
def M(D):
 d="y"
...
def O():
 ...
 K(j,U)
 M(D)
 m,s=divmod(int(E()-q),60)
 print(f"\n⌛ Total downloading time: \033[38;5;46m{m:02d}:{s:02d}\033[0m")
