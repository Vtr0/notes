...
The result will be a json on `window.cList` variable. It also being copied to clipboard, and can paste into [json-editor](https://jsoneditoronline.org/) for online formatting. The list of chapter that can't get the `duration` is in variable `window.dur0`.
